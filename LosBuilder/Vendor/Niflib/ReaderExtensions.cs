...
namespace Niflib
{
    ...
    public static class ReaderExtensions
    {
        ...
        {
            if (version < eNifVersion.v4_1_0_1)
        ...
        public static Matrix ReadMatrix33(this BinaryReader reader)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    SetMatrixValue(ref identity, j, i, reader.ReadSingle());
                }
            }
            return identity;
        }

        private static void SetMatrixValue(ref Matrix matrix, int row, int column, float value)
        {
#if OpenTK || SharpDX || MonoGame
