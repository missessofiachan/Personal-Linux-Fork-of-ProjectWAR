...
Generated UTC: `2026-03-28T07:55:47.6870721Z`
...
| --- | --- | --- | --- | --- | --- | --- | ---: |
| 1 | Coverage Gaps | Critical | 190 | Common missing pattern: csv, bin, effect-text, effect-row, components | csv, bin, effect-text, effect-row, components | Resolve the missing root effect link so ability flow can be traced through the client effect chain. | 986 |
...
| 8 | Coverage Gaps | High | 160 | StringsOnly ability bucket | StringsOnly | Find the missing BIN, effect, and component evidence so this stops being a text-only ability shell. | 986 |
| 9 | Coverage Gaps | High | 154 | Common missing pattern: bin, components | bin, components | Recover the missing component linkage before trying to interpret operation semantics. | 20 |
| 10 | Coverage Gaps | High | 145 | Partial ability bucket | Partial | Close the missing extracted-client pieces called out in the pattern list before spending time on deeper semantic decoding. | 4 |

## Area Summary

| Area | Items | Critical | High | Peak | Bucket | Summary | Next Step |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Coverage Gaps | 10 | 6 | 4 | 190 | Critical | 12664 abilities remain below `Mapped`; the largest buckets are Partial, StringsOnly. | Push the largest shared missing-pattern buckets first so ability reports move from sparse or string-only states into repeatable mapped states. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | ---: |
| 1 | 1 | Critical | 190 | Common missing pattern: csv, bin, effect-text, effect-row, components | csv, bin, effect-text, effect-row, components | 1028 abilities still share this extracted-client gap pattern. | Statuses: StringsOnly. Samples: 986, 1855, 5295, 5296, 5297, 5298, 5299, 5377, 5384, 5395, 5396, 5397. | Resolve the missing root effect link so ability flow can be traced through the client effect chain. | 986 |
...
| 8 | 8 | High | 160 | StringsOnly ability bucket | StringsOnly | 1029 abilities still sit in `StringsOnly` coverage instead of a fully mapped state. | Samples: 986, 999, 1855, 5295, 5296, 5297, 5298, 5299, 5377, 5384, 5395, 5396. Common missing pieces: csv, bin, effect-text, effect-row, components, csv, bin, effect-row, components. | Find the missing BIN, effect, and component evidence so this stops being a text-only ability shell. | 986 |
| 9 | 9 | High | 154 | Common missing pattern: bin, components | bin, components | 9 abilities still share this extracted-client gap pattern. | Statuses: Partial. Samples: 20, 121, 126, 1000, 1003, 1016, 1017, 1018, 1019. | Recover the missing component linkage before trying to interpret operation semantics. | 20 |
| 10 | 10 | High | 145 | Partial ability bucket | Partial | 11635 abilities still sit in `Partial` coverage instead of a fully mapped state. | Samples: 4, 20, 36, 39, 40, 50, 51, 52, 53, 62, 63, 64. Common missing pieces: bin, effect-text, components, bin, components, effect-text, effect-row. | Close the missing extracted-client pieces called out in the pattern list before spending time on deeper semantic decoding. | 4 |

## Conflict Hotspots Top Items

| Rank | Global | Priority | Score | Title | Subject | Summary | Evidence | Action | ExampleAbilityId |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | ---: |
| 1 | 11 | Medium | 110 | StringMismatch (AbilityDescription) | StringMismatch \| AbilityDescription | 16 non-noise conflicts across 16 subjects. | Peak triage: 85. High-signal rows: 0. Sample subjects: Ability:1900, Ability:1901, Ability:1902, Ability:1903, Ability:1904, Ability:1905. | Inspect the claim-level evidence and encode an explicit canonical rule for this disagreement. | 1900 |
| 2 | 12 | Medium | 110 | StringMismatch (AbilityName) | StringMismatch \| AbilityName | 2110 non-noise conflicts across 2110 subjects. | Peak triage: 85. High-signal rows: 0. Sample subjects: Ability:10, Ability:1008, Ability:1020, Ability:1053, Ability:1054, Ability:1064. | Separate player-facing names from internal placeholders and keep the canonical string rule explicit. | 10 |
| 3 | 13 | Low | 85 | StringMismatch (EffectName) | StringMismatch \| EffectName | 2 non-noise conflicts across 2 subjects. | Peak triage: 75. High-signal rows: 0. Sample subjects: Effect:3353, Effect:48. | Inspect the claim-level evidence and encode an explicit canonical rule for this disagreement. |  |
| 4 | 14 | Low | 52 | InternalOnlyAbilityNameMismatch (AbilityName) | InternalOnlyAbilityNameMismatch \| AbilityName | 4 non-noise conflicts across 4 subjects. | Peak triage: 32. High-signal rows: 0. Sample subjects: Ability:106, Ability:14, Ability:26, Ability:3266. | Separate player-facing names from internal placeholders and keep the canonical string rule explicit. | 106 |
